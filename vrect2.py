...
def object_ini():
    ...
    """
    niddleimg = pygame.image.load("aiguille.png")
    niddleimg = pygame.transform.scale(niddleimg, (WWALLV, WWALLV))
    tubeimg = pygame.image.load("tube_plastique.png")
...
def place_guard():
    ...
    guard = pygame.image.load("Gardien.png")
...
def load_win(continuing):
    """
        Function to load the win screen
    """
    while continuing != 0:
...
def mc_gyver_ini():
    """
        Initiate McGyver
    """
    mc_gyver_img = pygame.transform.scale(pygame.image.load("MacGyver.png"), (WWALLV, HWALLH))
